        ...
        <div class="ml-0 lg:ml-64">
            ...
            <header class="fixed top-0 right-0 left-0 lg:left-64 h-20 bg-white/95 border-b border-gray-200 flex items-center justify-between px-4 sm:px-8 z-40">
                <div>
                    ...
                    <p class="text-sm text-gray-400 mt-1">Kelola fitur unggulan Desa Terpadu</p>
                </div>
            ...
            <main class="p-4 sm:p-8 pt-24 sm:pt-28 min-h-screen">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Kelola informasi platform dan fitur Desa Terpadu.</p>
                </div>
                ...
                <?php if (!empty($platforms)): ?>
                    ...
                    <?php foreach ($platforms as $platform): ?>
                        ...
                        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden mb-6">
                            ...
                            <!-- Platform Header -->
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 px-6 py-5 border-b border-gray-200">
                                <div class="flex gap-4">
                                    <div class="w-12 h-12 min-w-[48px] bg-red-50 text-red-500 rounded-xl flex items-center justify-center">
                                        <!-- Ikon Star -->
                                    ...
                                    <div>
                                        <h2 class="text-lg font-bold text-gray-800"><?= html_escape($platform->name); ?></h2>
                                        <p class="mt-1 text-sm text-gray-600 leading-relaxed max-w-2xl">
                                ...
                                <!-- Edit Platform -->
                                <a href="<?= site_url('admin/features/edit-platform/' . $platform->id); ?>"
                                    class="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 rounded-lg text-sm font-semibold hover:bg-amber-100 transition whitespace-nowrap self-start">
                                    <!-- Ikon Pencil -->
                            ...
                            <div class="p-6">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="text-sm font-semibold text-gray-700">Daftar Fitur</h3>
                                </div>

                                <?php
                                $feature_count = 0;
                                $platform_items = array_filter($items, function($item) use ($platform) {
                                ...
                                <?php if (!empty($platform_items)): ?>
                                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                        <?php foreach ($platform_items as $item): ?>
                                            <?php $feature_count++; ?>
                                            <div class="border border-gray-200 rounded-xl p-4 bg-white hover:border-red-200 hover:shadow-md transition">
                                                <div class="flex items-start justify-between gap-3">
                                                    <div class="w-8 h-8 bg-red-50 text-red-500 rounded-lg flex items-center justify-center text-xs font-bold">
                                                        <?= $feature_count; ?>
                                                    </div>
                                                    <a href="<?= site_url('admin/features/edit-item/' . $item->id); ?>"
                                                        class="text-xs font-semibold text-gray-500 hover:text-red-500 transition">
                                                        Edit
                                                    </a>
                                                </div>
                                                <h4 class="mt-3 text-sm font-semibold text-gray-800 leading-snug">
                                ...
                                <?php else: ?>
                                    <!-- Empty Feature dalam Platform -->
                                    <div class="py-10 text-center border border-dashed border-gray-300 rounded-xl text-gray-400">
                                        <div class="w-12 h-12 mx-auto mb-3 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center">
                                            <!-- Ikon Star -->
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.196-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                                            </svg>
                                        </div>
                                        <p class="text-sm">Belum ada fitur untuk platform ini.</p>
                                    </div>
                                <?php endif; ?>
                            </div>
                ...
                <?php else: ?>
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                        <div class="py-16 text-center">
                            <div class="w-16 h-16 mx-auto mb-4 bg-red-50 text-red-500 rounded-full flex items-center justify-center">
                                <!-- Ikon Star -->
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.196-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                                </svg>
                            </div>
                            <strong class="block text-sm font-semibold text-gray-700">Belum Ada Platform</strong>
                            <p class="mt-2 text-sm text-gray-500">Belum ada platform yang tersedia.</p>
                        </div>
                    </div>
